...
//        app::Application application(game, ioc, args->tick_period,
//                                     args->randomize_spawn_points);

        // 3. Загрузка сохраненной игры


        //fs::path game_save_path;
       //std::chrono::milliseconds save_period;

        if (!args->state_file.empty()) {
            //game_save_path = args->state_file;
...
//        }
        ...
        auto api_handler = std::make_shared<http_handler::ApiRequestHandler>(*application, static_path);    //application
